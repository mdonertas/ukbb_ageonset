...
## ageonset/disRates_50000_95_all

* Generated by ./scripts/12-age_of_onset_clusters.R
* Uses ./data/processed/ageonset/permRes_50000_all.rds
* For each disease, we plot tthe number of participants in 10,000, who are diagnosed with that disease at a certain age (x-axis). Diseases are ordered and colored by the disease category. Each point shows median number of individuals who aquired that disease at that age based on 100 permutations. Segments on top of the points show the 95% range of the data points. A best fit curve (calculated using loess regression) is also displayed.

## ageonset/disRates_50000_95

* Generated by ./scripts/12-age_of_onset_clusters.R
* The same as disRates_50000_95_all but only for the 116 diseases we are analysing. 

## ageonset/disRates_50000_sd

* Generated by ./scripts/12-age_of_onset_clusters.R
* The same as disRates_50000_95 except the points show mean of the values from permutations and segments show sd instead of 95% range.

## ageonset/medAgeRate

* Generated by ./scripts/12-age_of_onset_clusters.R
* A heatmap showing age of onset distributions for the 116 diseases we analyse, based on the median number of people based on repeated samplings. 
* Columns show 0-64 years of age, and rows are different diseases clustered by hclust function in R based on euclidean distanced between age-of-onset distributions. The color shows the proportion of people who acquired that particular disease among all people acquired in 10,000.

## ageonset/pca_medRate_agecolored

* Generated by ./scripts/12-age_of_onset_clusters.R
* PCA plot summarising PC1 and PC2, and median Age of onset. Values in parantheses gives the variance explained by each PC. Color shows the median age-of-onset for each disease, where brighter colors mean young and darker colors mean old age. 

## ageonset/pca_medRate_catcolored

* Generated by ./scripts/12-age_of_onset_clusters.R
* The same plot as pca_medRate_agecolored but this time colored by disease categories. Size shows the median age of onset, where smaller circles indicate early onset diseases.

## ageonset/topdat

* Generated by ./scripts/12-age_of_onset_clusters.R
* Summary plot showing the age at first disease diagnosed. Again, this plot is calculated using random sampling of 50,000 people so that data is not biased by the number of individuals after certain age. Points show median age of onset based on those samplings and the segments show 95% range. Best fit curve, calculated using loess regression, is also depicted. 

## ageonset/*cluster/

* Generated by ./scripts/12-age_of_onset_clusters.R
* Hierarchical clustering of disease age of onset data based on median values of 100 samplings are calculated. Then for k=2,..,25, we visualise the k clusters on the tree and summary age-of-onset distributions for each cluster - where the line shows mean value for diseases at each age and gray segments show sd. 
* Plots for each cluster are drawn separately as n.jpeg and dendrogram with cluster summaries are plotted as k.pdf and k.png

## ageonset/allsummarised

* Generated by ./scripts/12-age_of_onset_clusters.R
* Summary plot showing median age of onset and the range between the first and third quartile for each disease. Diseases are first ordered by the category and within each category they are ordered by the median age of onset. The points show median value whereas the segment shows the range between the first and third quartile.

## ageonset/allsummarised2

* Generated by ./scripts/12-age_of_onset_clusters.R
* The same as allsummarised but not ordered by the category so that we can see if certain colors are more in the ends.

## ageonset/catsummarised

* Generated by ./scripts/12-age_of_onset_clusters.R
* Median age of onset values for each disease grouped by the categories. Each dot shows a disease, and boxplots summarised each category. 

## ageonset/clusterColoredCat

* Generated by ./scripts/12-age_of_onset_clusters.R
* Age of onset clustering colored by disease categories. 

## ageonset/age_cat_overlap_k*

* Generated by ./scripts/12-age_of_onset_clusters.R
* A heatmap showing what percent of a certain age of onset cluster is composed of a certain category. Rows are ordered by the occurrence in disease onset cluster tree and columns are ordered by hieerarchical clustering. The color shows the percentage and the percentages are also written. 