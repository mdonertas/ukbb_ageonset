...
unimap = lapply(unisx,function(unix){
  print(unix)
  uni = readRDS(unix)
...
unimap = reshape2::melt(unimap,id.vars = colnames(unimap[[1]])) %>%
  select(-L1) %>%
  as.tibble()
print('unimap is created')
unimap2 = list()
for( i in 1:nrow(unimap)){
  print(i)
  x = unimap[i,]
  ...
  unimap2[[i]] = as.data.frame(x) %>%
    mutate(pos = pos, newGenSeq = toupper(genseq2), genChange =genChange, newAA = newAA,type = type, ismatch_seq =ismatch_seq)
  saveRDS(unimap2,'./temp/unimap.rds')
}
print('unimap2 is created')
unimap2 = reshape2::melt(unimap2,id.vars = colnames(unimap2[[1]])) %>%
  select(-L1)
print('unimap2 is melted')
# system('mkdir ./data/processed/codingVariants')
saveRDS(unimap2,'./data/processed/codingVariants/unimap.rds')
print('unimap2 is saved under ./data/processed/codingVariants/unimap.rds')
paste('finished')
