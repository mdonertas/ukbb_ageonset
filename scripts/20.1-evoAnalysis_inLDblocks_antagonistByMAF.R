...
for(mafx in c(19:0/20)){
  ...
  antaplot[[k]] = ld_cl1cl2_anta %>% 
    ...
    ggtitle(paste( '>= ',mafx*100,' % BETA',sep=''))
  print(antaplot[[k]])
}
p1=ggarrange(plotlist = antaplot,ncol=5,nrow=4,labels='auto')
ggsave('./results/evoAnalysis/cl12_antagonist_maffiletrs.pdf',p1, width = 28,height = 24, units = 'cm',useDingbats=F)
ggsave('./results/evoAnalysis/cl12_antagonist_maffiletrs.png',p1, width = 28,height = 24, units = 'cm')
